...
fileprivate enum Constants {
  ...
  static let instructionsIconWidth: CGFloat = 24
...
fileprivate enum Strings {
  ...
  static let openSafariSettings = "Open Safari Settings"
  static let clickOn = "Click on "
  static let clickThe = "Click the "
  static let nextTo = " next to "
  static let ghostery = " Ghostery"
  static let allowInPrivate = "Allow in private mode (optional but recommended)"
  static let navigateToWebsite = "Go to any website on Safari"
  static let clickGhostery = "Click the Ghostery icon "
  static let withThe = " with the "
  static let clickEnable = "Click ‘Enable Ghostery’ "
}
...
struct WelcomeView: View {
  ...
  var macSteps: some View {
    VStack(alignment: .listRowSeparatorLeading, spacing: Constants.noSpacing) {
      step(number: 1, textBeforeIcon: Strings.openSafariSettings)
      Spacer()
      step(number: 2, textBeforeIcon: Strings.clickOn, iconName: Icons.plugins, textAfterIcon: Strings.toManageExtensions)
      Spacer()
      step(number: 3, textBeforeIcon: Strings.clickThe, iconName: Icons.checkmark, textAfterIcon: Strings.nextTo, secondIconName: Icons.ghosterySmall, secondTextAfterIcon: Strings.ghostery)
      Spacer()
      HStack(alignment: .top, spacing: Constants.noSpacing) {
        Text(Strings.step + " \(4). ")
          .font(Fonts.text)
          .foregroundStyle(theme == .light ? Colors.lightTextColor: Colors.darkTextColor)
        Text(Strings.allowInPrivate)
          .font(Fonts.text)
          .foregroundStyle(theme == .light ? Colors.lightTextColor: Colors.darkTextColor)
      }
      Spacer()
      step(number: 5, textBeforeIcon: Strings.navigateToWebsite)
      Spacer()
      step(number: 6, textBeforeIcon: Strings.clickGhostery, iconName: Icons.ghosterySmallGray, textAfterIcon: Strings.withThe, secondIconName: Icons.attention)
      Spacer()
      step(number: 7, textBeforeIcon: Strings.clickEnable, iconName: Icons.click)
    }
  }
  
  func step(number: Int,
            textBeforeIcon: String,
            iconName: String? = nil,
            textAfterIcon: String? = nil,
            secondIconName: String? = nil,
            secondTextAfterIcon: String? = nil) -> some View {
    HStack(alignment: .center, spacing: Constants.noSpacing) {
      Text(Strings.step + " \(number). " + textBeforeIcon)
        .font(Fonts.text)
        .foregroundStyle(theme == .light ? Colors.lightTextColor: Colors.darkTextColor)
      ...
      if let secondIconName = secondIconName {
        Image(secondIconName)
          .resizable()
          .scaledToFit()
          .frame(width: Constants.instructionsIconWidth)
      }
      if let secondTextAfterIcon = secondTextAfterIcon {
        Text(secondTextAfterIcon)
          .font(Fonts.text)
          .foregroundStyle(theme == .light ? Colors.lightTextColor: Colors.darkTextColor)
      }
      Spacer()

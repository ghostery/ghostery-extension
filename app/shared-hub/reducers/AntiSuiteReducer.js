...
function AntiSuiteReducer(state = initialState, action) {
	switch (action.type) {
		case SET_AD_BLOCK: {
			...
			return { ...state, anti_suite: { ...state.anti_suite, enable_ad_block } };
		}
		case SET_ANTI_TRACKING: {
			const { enable_anti_tracking } = action.data;
			return { ...state, anti_suite: { ...state.anti_suite, enable_anti_tracking } };
		}
		case SET_SMART_BLOCK: {
			const { enable_smart_block } = action.data;
			return { ...state, anti_suite: { ...state.anti_suite, enable_smart_block } };
		}

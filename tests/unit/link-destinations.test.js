...
import { isResultPage, getDestination, hash } from '../../src/utils/link-destinations.js';
...
describe('link destinations', () => {
  ...
  describe('hash()', () => {
    it('gives the same key for the same text', () => {
      assert.equal(hash('A result title'), hash('A result title'));
    });

    it('gives another key for another text', () => {
      assert.notEqual(hash('A result title'), hash('A result title.'));
      assert.notEqual(hash('ab'), hash('ba'));
      assert.notEqual(hash(''), hash(' '));
    });

    it('keeps the key short whatever the text', () => {
      for (const text of ['', 'a', 'A result title - example.com › a-result', 'x'.repeat(10000)]) {
        assert.match(hash(text), /^[0-9a-z]{1,11}$/);
      }
    });

    // Keys are stored across sessions, so the function must not change output
    it('is stable across versions', () => {
      assert.equal(hash(''), 'wvjl67o803');
      assert.equal(hash('hello'), '19jqrdkps12');
    });
  });
});

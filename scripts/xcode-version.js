...
const path = resolve(process.cwd(), './xcode/Ghostery.xcodeproj/project.pbxproj');
...
file = file
  .replace(/CURRENT_PROJECT_VERSION = (\d+);/g, `CURRENT_PROJECT_VERSION = ${buildVersion};`)
  .replace(/MARKETING_VERSION = (\d+\.\d+\.\d+);/g, `MARKETING_VERSION = ${version};`);
